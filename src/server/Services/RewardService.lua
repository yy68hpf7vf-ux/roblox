...
function RewardService.dailyView(player: Player, profile: Profile): { [string]: any }
	...
	return {
		...
		nextStreak = upcoming,
		label = rung.label,
		crystals = EconomyService.secondsToCrystals(player, profile, rung.seconds, 250),
		cores = rung.cores or 0,
	}
end
...
function RewardService.claimDaily(player: Player, profile: Profile): (boolean, string)
	...
	local crystals = EconomyService.secondsToCrystals(player, profile, rung.seconds, 250)

	profile.daily.streak = streak
	profile.daily.lastClaimAt = os.time()

	EconomyService.addCrystals(player, profile, crystals)
	if rung.cores then
		EconomyService.addCores(player, profile, rung.cores)
	end
...
function RewardService.playtimeView(player: Player, profile: Profile): { [string]: any }
	...
	return {
		...
		nextCrystals = if rung then EconomyService.secondsToCrystals(player, profile, rung.seconds, 150) else 0,
...
function RewardService.claimPlaytime(player: Player, profile: Profile): (boolean, string)
	...
	local crystals = EconomyService.secondsToCrystals(player, profile, rung.seconds, 150)
	EconomyService.addCrystals(player, profile, crystals)
