...
--[[
	...
	Two things this file takes seriously:
	  ...
	  * Receipts are granted exactly once. ProcessReceipt can fire more than once
	    ...
	    nothing.
	  * Crystal packs are sized against the buyer's own income and the exact
	    number is on the button before the prompt opens. Nobody should have to
	    guess what they are getting, and nobody should find out their pack was
	    worth four minutes at the rate they now earn.
]]
...
--[[ What a crystal pack is worth to this player right now. The store calls this
     through the state payload so the button and the grant always agree. ]]
function MonetizationService.crystalAmount(player: Player, profile: Profile, product: Monetization.Product): number
	return EconomyService.secondsToCrystals(player, profile, product.seconds or 0, product.floor)
end

local function grant(player: Player, profile: Profile, product: Monetization.Product): boolean
	if product.kind == "crystals" then
		local amount = MonetizationService.crystalAmount(player, profile, product)
		EconomyService.addCrystals(player, profile, amount)
		Remote.notify(player, `{Format.comma(amount)} Crystals added. Thank you.`, "good")
		return true
	elseif product.kind == "boost" then
		...
		Remote.notify(
			player,
			`{Format.multiplier(product.multiplier or 2)} Crystals for {Format.duration(EconomyService.boostRemaining(profile))}.`,
			"good"
		)
		return true
	end
