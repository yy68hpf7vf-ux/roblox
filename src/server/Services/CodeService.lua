...
function CodeService.redeem(player: Player, profile: Profile, input: unknown): (boolean, string)
	...
	if entry.seconds then
		local crystals = EconomyService.secondsToCrystals(player, profile, entry.seconds, 500)
		EconomyService.addCrystals(player, profile, crystals)
		table.insert(parts, `{Format.short(crystals)} Crystals`)
	end
	if entry.cores then
		EconomyService.addCores(player, profile, entry.cores)
		table.insert(parts, `{entry.cores} Cores`)
	end
