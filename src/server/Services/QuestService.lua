...
--[[
	...
	Three quests, rolled from the day number so every server agrees and rejoining
	cannot reroll them. Progress is server-side only -- the client never reports a
	quest as done, it just renders what the server sends.
]]
...
--[[ Targets and rewards are resolved fresh every time rather than frozen into
     the save, so a player who rebirths mid-day sees the quest scale with them
     instead of being stuck on yesterday's numbers. ]]
function QuestService.targetFor(player: Player, profile: Profile, def: Quests.QuestDef): number
	if def.kind == "earn_crystals" then
		return EconomyService.secondsToCrystals(player, profile, Quests.EarnCrystalsSeconds, 1000)
	end
	return Quests.targetFor(def, profile.rebirths)
end

function QuestService.rewardFor(player: Player, profile: Profile, def: Quests.QuestDef): number
	return EconomyService.secondsToCrystals(player, profile, def.rewardSeconds, 500)
end
...
function QuestService.claim(player: Player, profile: Profile, questId: string): (boolean, string)
	...
	for _, state in profile.quests.list do
		if state.id == questId then
			...
			local reward = QuestService.rewardFor(player, profile, def)
			state.claimed = true
			EconomyService.addCrystals(player, profile, reward)
