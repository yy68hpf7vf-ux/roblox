	...
	local function refresh(state)
		...
		if daily.available then
			loginDetail.Text = `Streak {daily.streak or 0} -> {daily.nextStreak or 1}.`
				.. `\nWorth {Format.short(daily.crystals or 0)} Crystals`
				.. (if (daily.cores or 0) > 0 then ` and {daily.cores} Cores.` else ".")
		...
		elseif play.canClaim then
			playDetail.Text = `{claimed}/{total} collected. Next up: {play.nextLabel} for {Format.short(play.nextCrystals or 0)} Crystals.`
		...
		for index, quest in state.quests or {} do
			...
			row.desc.Text = `{Format.short(quest.progress)} / {Format.short(quest.target)}  ·  pays {Format.short(quest.reward)} Crystals`
